...
// 1. LẤY DANH MỤC (Để hiện Sidebar)
...
$whereConditions = [];
$param = ''; // Chuỗi để giữ tham số trên URL
...
// C. Lọc theo Nơi bán (DÙNG LIKE ĐỂ TÌM TRONG CHUỖI ĐỊA CHỈ)
// Logic: Value gửi lên là "Hà Nội", "Huế"... -> Tìm chuỗi này trong database
if (!empty($_GET['place'])) {
    $place_sql_parts = [];
    foreach ($_GET['place'] as $p) {
        // Chỉ lấy các ký tự an toàn
        $p_clean = mysqli_real_escape_string($conn, $p);
        
        // Tìm địa chỉ shop CHỨA từ khóa tỉnh thành
        // VD: shops.address LIKE '%Hà Nội%' sẽ tìm thấy '... Thành phố Hà Nội'
    ...
    if (!empty($place_sql_parts)) {
        // Nối bằng OR: Ở Hà Nội HOẶC ở HCM đều lấy
...
// 4. THỰC THI QUERY (JOIN BẢNG SHOPS)
// Phải JOIN để lấy được cột address của shop
                ...
                <form id="filters-form" method="get" action="index.php">
                    ...
                    <section class="filter-block">
                        ...
                        <div id="province-list" class="province-scroll-box" style="max-height: 200px; overflow-y: auto; border: 1px solid #eee; padding: 10px; border-radius: 4px;">
                            <p style="color:#999; font-size:12px;">Đang tải địa điểm...</p>
                        </div>
                    ...
                    <a href="index.php?controller=product&action=list" style="display:block; text-align:center; margin-top:10px; color:#555; text-decoration:none; font-size: 13px;">XÓA TẤT CẢ</a>
                </form>
            </aside>
            ...
            <section class="content">
                <div class="sort-bar">
                    <div class="sort-left">
                        ...
                        <a href="index.php?controller=product&action=list&<?= build_full_url(['sort'=>'']) ?>" class="btn-sort <?= $sort=='' ? 'active':'' ?>">Liên Quan</a>
                        <a href="index.php?controller=product&action=list&<?= build_full_url(['sort'=>'new']) ?>" class="btn-sort <?= $sort=='new' ? 'active':'' ?>">Mới Nhất</a>
                    </div>
                    <div class="sort-right">
                        <select onchange="location = this.value;" style="padding:5px; border:1px solid #ddd;">
                            <option value="index.php?controller=product&action=list&<?= build_full_url(['sort'=>'']) ?>">Giá: Mặc định</option>
                            <option value="index.php?controller=product&action=list&<?= build_full_url(['sort'=>'price_asc']) ?>" <?= $sort=='price_asc'?'selected':'' ?>>Giá: Thấp đến Cao</option>
                            <option value="index.php?controller=product&action=list&<?= build_full_url(['sort'=>'price_desc']) ?>" <?= $sort=='price_desc'?'selected':'' ?>>Giá: Cao đến Thấp</option>
                        </select>
                    </div>
                </div>

                <?php if($keyword): ?>
                    <div style="margin-bottom:15px; color:#555;">Kết quả cho: <strong style="color:#ee4d2d"><?= htmlspecialchars($keyword) ?></strong> (<?= $total_records ?>)</div>
                <?php endif; ?>
            ...
            $.getJSON('https://esgoo.net/api-tinhthanh/1/0.htm', function(response) {
                if (response.error == 0) {
                    ...
                    $.each(response.data, function(key, val) {
                        var fullName = val.full_name; // VD: Thành phố Hà Nội
                        
                        // KỸ THUẬT QUAN TRỌNG: Cắt bỏ chữ "Tỉnh", "Thành phố"
                        // Để khi tìm kiếm LIKE trong database sẽ dễ trúng hơn
                        // VD: Database lưu "Thành phố Hà Nội" -> Tìm "Hà Nội" vẫn ra.
                        var cleanName = fullName.replace(/^(Tỉnh|Thành phố)\s+/, "");
                        
                        // Kiểm tra xem tỉnh này có đang được chọn không (để tích V)
                    ...
                    $('#province-list').html('<p style="color:red">Lỗi tải danh sách</p>');
                }
            });
        });
    </script>
